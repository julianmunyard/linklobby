...
must_haves:
  truths:
    ...
    - "Display name changes update profile store"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/components/editor/header-section.tsx"
      to: "src/stores/profile-store.ts"
      via: "display name Input onChange calls setDisplayName"
      pattern: "setDisplayName.*value|onChange.*setDisplayName"
---
...
7. **Display name Input wiring (CRITICAL):**
   - Get displayName from useProfileStore()
   - Get setDisplayName from useProfileStore()
   - Input value={displayName || ''}
   - Input onChange={(e) => setDisplayName(e.target.value)}
   - This connects user typing to the store for live preview updates

8. Crop dialog integration:
...
- Handle errors with console.error (toast can be added later)
  ...
  <done>HeaderSection renders photo, layout toggle, title style toggle, conditional fields with display name wired to store</done>
</task>
...
- Display name Input onChange updates profileStore.setDisplayName
</verification>
...
- Display name input wired: typing calls profileStore.setDisplayName(value)
</success_criteria>
