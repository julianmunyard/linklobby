...
<task type="auto">
  <name>Task 1: Fix DropdownCard broken toggle and event blocking</name>
  <files>src/components/cards/dropdown-card.tsx</files>
  <action>
The current DropdownCard implementation is BROKEN with these root cause bugs:
- Line 16: Local `useState` for `isOpen` causes state corruption during drag transforms
- Line 26: `stopPropagation` on `onClick` blocks interactions
- Line 37: `stopPropagation` on `onPointerDown` completely blocks dnd-kit drag

FIX by refactoring to a controlled component that receives open/close state from parent:
