...
must_haves:
  truths:
    ...
    - "Game plays are recorded when visitors play game cards on a public page"
    - "Gallery image views are recorded when visitors interact with photo gallery cards"
    - "Unique visitors are counted using privacy-safe hashed fingerprints (not cookies)"
    - "Analytics stats API returns unique visitors, total views, per-card click counts with CTR, game play counts, and gallery view counts"
  artifacts:
...
<task type="auto">
  ...
  <action>
    1. Create `supabase/migrations/analytics_tables.sql` with:
       ...
       - `analytics_interactions` table: time (TIMESTAMPTZ NOT NULL), card_id (UUID NOT NULL REFERENCES cards(id) ON DELETE CASCADE), page_id (UUID NOT NULL REFERENCES pages(id) ON DELETE CASCADE), visitor_hash (TEXT NOT NULL), interaction_type (TEXT NOT NULL CHECK (interaction_type IN ('game_play', 'gallery_view')))
       - Enable RLS on all three tables. Policy: INSERT allowed by anyone (anon role for public tracking). SELECT only by page owner (auth.uid() matches page user_id).
       - Create indexes: (page_id, time DESC) on page_views, (card_id, time DESC) on card_clicks, (card_id, interaction_type, time DESC) on interactions
    ...
    3. Create `src/lib/analytics/track-event.ts`:
       - Export helper types: `TrackPageViewPayload { pageId: string, pathname: string }`, `TrackCardClickPayload { cardId: string, pageId: string }`, and `TrackInteractionPayload { cardId: string, pageId: string, interactionType: 'game_play' | 'gallery_view' }`
       - Export `trackPageView(payload)`, `trackCardClick(payload)`, and `trackInteraction(payload)` functions that POST to `/api/analytics/track`

    4. Create `src/app/api/analytics/track/route.ts`:
       - POST handler accepts JSON body with `type` field ('page_view', 'card_click', or 'interaction')
       ...
       - For interaction: extract cardId, pageId, interactionType from body. Create visitor hash. Insert into analytics_interactions with interaction_type ('game_play' or 'gallery_view').
    ...
    5. Create `src/app/api/analytics/stats/route.ts`:
       ...
       - Query interaction stats: JOIN cards with analytics_interactions grouped by card_id and interaction_type, COUNT(*) as count per card per type. Merge into cardStats as `gamePlays` (for game cards) and `galleryViews` (for gallery cards).
       - Query time-series: Group page views by day (DATE_TRUNC), return array of { date, views, visitors } for charting
       - Return JSON: { uniqueVisitors, totalViews, cardStats: [{ id, title, cardType, clicks, ctr, gamePlays?, galleryViews? }], timeSeries: [{ date, views, visitors }] }
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
    1. Create `src/components/public/click-tracker.tsx`:
       ...
       - Also listen for custom events for specialized interactions:
         - `data-game-play` attribute on game play buttons: when clicked, send type 'interaction' with interactionType 'game_play'
         - `data-gallery-view` attribute on gallery image elements: when clicked/opened, send type 'interaction' with interactionType 'gallery_view'
    ...
    2. Modify `src/app/[username]/page.tsx`:
       ...
       - Add data-game-play={card.id} on game card play buttons and data-gallery-view={card.id} on gallery card image elements so ClickTracker can detect these specialized interactions.
  ...
  <done>
    Visiting a public page records a page view. Clicking any card records a card click. Playing a game records a game_play interaction. Opening a gallery image records a gallery_view interaction. All tracking is non-blocking and gracefully handles failures.
  </done>
</task>
