...
1. In `src/components/editor/featured-themes-tab.tsx`:
   - Find the click handler on featured theme cards (likely calls `applyTemplate()` or similar directly)
   - Replace the direct template application with a call to `onNavigateToTheme(themeId)` — this prop should already be available from EditorPanel (see research: "The `onNavigateToTheme` prop already flows into `FeaturedThemesTab` from `EditorPanel`")
   - If `onNavigateToTheme` is NOT already passed as a prop, check EditorPanel and add it to the props passed to FeaturedThemesTab
