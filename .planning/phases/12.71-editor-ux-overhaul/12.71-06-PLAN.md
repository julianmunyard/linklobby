...
3. In `src/components/cards/card-renderer.tsx`:
   - Add an `isEditable?: boolean` prop to CardRenderer's props interface
   - In the TextCard render case, pass `isEditable={isEditable}` through to TextCard
   - This is required because the preview page renders cards via CardRenderer, not directly

4. In `src/app/preview/page.tsx`:
   - Pass `isEditable={true}` to CardRenderer when rendered inside the editor iframe
...
5. IMPORTANT: Do NOT modify the public page rendering. The public page components (StaticFlowGrid etc.) should continue passing `isEditable={false}` (default) or not passing the prop at all.

6. Use `e.stopPropagation()` on the InlineEditable wrapper to prevent the card click (SELECT_CARD) from firing when the user clicks to start editing.
  </action>
  <verify>
