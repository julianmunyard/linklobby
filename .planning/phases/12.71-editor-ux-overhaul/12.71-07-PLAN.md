...
3. Scatter mode: Skip for now — floating toolbar is flow-layout only in this phase. Scatter mode cards use absolute positioning on ScatterCanvas with different coordinate systems; adding toolbar support there requires separate investigation. If `isScatterMode` is available, conditionally hide the toolbar.
