...
3. Send INLINE_EDIT_ACTIVE/DONE messages to suppress deselect:
   - InlineEditable already has `onEditStart` and `onEditEnd` props (added in Plan 01). Use them:
     ```
     onEditStart={() => window.parent.postMessage({ type: 'INLINE_EDIT_ACTIVE' }, window.location.origin)}
     onEditEnd={() => window.parent.postMessage({ type: 'INLINE_EDIT_DONE' }, window.location.origin)}
     ```
