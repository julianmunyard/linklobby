...
export function BackgroundControls() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Reuse pattern from profile image upload
...
Also add uploadBackgroundImage function to storage.ts if not exists.
...
- Video URL input with preview
  ...
  <verify>
Run `npx tsc --noEmit`. Check export: `grep "export function BackgroundControls" src/components/editor/background-controls.tsx`
  </verify>
  <done>
BackgroundControls created with solid color, image upload, and video URL options.
  </done>
</task>
