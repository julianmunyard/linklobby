...
Create reusable ColorPicker component following the pattern from RESEARCH.md:
...
Also create the debounce hook if it doesn't exist:
...
- Accessible with sr-only label
  ...
  <verify>
Run `npm install react-colorful` then `npx tsc --noEmit`. Check package.json includes react-colorful. Check export: `grep "export function ColorPicker" src/components/ui/color-picker.tsx`
  </verify>
  <done>
react-colorful installed. ColorPicker component created with popover, hex input, and debounced updates.
  </done>
</task>
