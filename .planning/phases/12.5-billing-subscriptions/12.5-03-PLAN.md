...
must_haves:
  truths:
    ...
    - "PlanTierProvider context owns planTier and upgrade modal state; ProGate opens UpgradeModal via context"
  ...
  artifacts:
    - path: "src/contexts/plan-tier-context.tsx"
      provides: "React context holding planTier and upgrade modal open state"
      exports: ["PlanTierProvider", "usePlanTier"]
  ...
  key_links:
    - from: "src/contexts/plan-tier-context.tsx"
      to: "src/components/billing/upgrade-modal.tsx"
      via: "PlanTierProvider owns upgradeModal open state, renders UpgradeModal at shell level"
      pattern: "UpgradeModal"
    - from: "src/components/billing/pro-gate.tsx"
      to: "src/contexts/plan-tier-context.tsx"
      via: "ProGate's onUpgradeClick calls context's openUpgradeModal"
      pattern: "openUpgradeModal"
    - from: "src/components/billing/pro-gate.tsx"
...
<task type="auto">
  ...
  <files>src/components/billing/pro-gate.tsx, src/contexts/plan-tier-context.tsx, src/components/editor/card-property-editor.tsx, src/components/editor/cards-tab.tsx, src/components/editor/design-panel.tsx</files>
  <action>
...
**Step 2: Create `src/contexts/plan-tier-context.tsx` — PlanTierProvider context.**

This context owns the plan tier state AND the upgrade modal open state for the editor. It eliminates prop drilling and wires ProGate to UpgradeModal.
...
**Step 2b: Wrap the editor shell with PlanTierProvider.**

Read the editor layout or page (e.g., `src/app/(dashboard)/editor/page.tsx` or `src/app/(dashboard)/layout.tsx`) — Plan 02 already established `planTier` fetching in the dashboard layout via `getUserPlan()`. Wrap the editor's client shell component in `<PlanTierProvider planTier={planTier}>`. This ensures:
1. All editor components can call `usePlanTier()` instead of prop drilling
2. ProGate can call `openUpgradeModal(feature)` via context
3. UpgradeModal is rendered ONCE at the shell level (not duplicated per ProGate instance)

**Step 2c: Wire ProGate's `onUpgradeClick` to context.**

Update ProGate to accept an optional `onUpgradeClick` prop OR consume `usePlanTier()` context directly. The simpler approach: make ProGate use the context internally:

```tsx
// In pro-gate.tsx, add:
import { usePlanTier } from '@/contexts/plan-tier-context'

// Inside ProGate component:
const { openUpgradeModal } = usePlanTier()

// Badge onClick:
<button onClick={() => openUpgradeModal(feature)} className="...">
  <Crown className="h-2.5 w-2.5" /> Pro
</button>
```

This means ProGate no longer needs `onUpgradeClick` or `currentTier` props — it reads both from context. Update ProGate props to only require `children`, `requiredTier?`, `feature?`, and `className?`. The `currentTier` and `onUpgradeClick` props become optional overrides (for use outside the editor context).

**Step 3: Wrap Pro features in the editor with ProGate.**
...
NOTE: Do NOT prop-drill planTier — use `usePlanTier()` from the PlanTierProvider context created in Step 2. All editor components should call `const { planTier, openUpgradeModal } = usePlanTier()` instead of accepting planTier as a prop.
  </action>
  <verify>
- `ls src/components/billing/pro-gate.tsx src/contexts/plan-tier-context.tsx` — components exist
- `grep "PlanTierProvider" src/contexts/plan-tier-context.tsx` — context provider exists
- `grep "usePlanTier" src/contexts/plan-tier-context.tsx` — hook exported
- `grep "UpgradeModal" src/contexts/plan-tier-context.tsx` — modal rendered at provider level
- `grep "openUpgradeModal" src/components/billing/pro-gate.tsx` — ProGate wired to open modal via context
...
- Comprehensive layout sweep — run `grep -r "hasProAccess" src/components/public/` and verify ALL layout files that render "Powered by LinkLobby" have the prop. Expected matches: static-flow-grid.tsx, static-scatter-canvas.tsx, static-phone-home-layout.tsx, static-macintosh-layout.tsx, static-ipod-classic-layout.tsx, static-vcr-menu-layout.tsx, static-receipt-layout.tsx, static-word-art-layout.tsx, static-chaotic-zine-layout.tsx, static-artifact-layout.tsx, public-page-renderer.tsx. If any layout file contains "Powered by LinkLobby" but NOT "hasProAccess", it was missed — fix it.
- `npx tsc --noEmit` — no type errors
  </verify>
  <done>
Public page calls getUserPlan once and strips Pro features for free users. Email collection and release cards are filtered out. Link scheduling metadata is stripped. "Powered by LinkLobby" footer only appears for free users across all layout files (verified via comprehensive grep sweep). Pro-only themes fall back to instagram-reels on free users' public pages. Legal links always display regardless of plan.
  </done>
</task>
