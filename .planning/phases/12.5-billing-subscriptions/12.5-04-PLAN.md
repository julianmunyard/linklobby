...
**Test 8: Graceful Degradation (Cancel Flow)**
1. As a Pro user, go to Settings and click "Manage Billing"
2. In the Stripe Customer Portal, cancel the subscription
3. Verify: Stripe CLI shows `customer.subscription.updated` webhook with `cancel_at_period_end: true`
4. Verify: Settings page shows "Your Pro plan ends on {date}" (not "renews")
5. Verify: Pro features STILL WORK during the remaining period (badge stays "Pro", public page still has Pro features, no branding footer)
6. To simulate period end: In Stripe Dashboard, manually advance the subscription clock or directly update the subscription status to 'canceled' in the local `subscriptions` table
7. Refresh the app — verify: plan badge reverts to "Free", public page shows "Powered by LinkLobby", email collection cards are stripped, Pro-only theme falls back to default
8. Settings page should show "Upgrade Plan" link (not "Manage Billing") since the user is now effectively free
  </how-to-verify>
...
- All 8 test scenarios pass (including graceful degradation cancel flow)
</success_criteria>
