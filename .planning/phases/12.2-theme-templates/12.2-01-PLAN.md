...
must_haves:
  truths:
    ...
    - "Template asset files exist on disk for the placeholder template"
  artifacts:
    ...
    - path: "public/templates/instagram-reels-dark-minimal/thumbnail.jpg"
      provides: "Thumbnail image for template picker grid"
    - path: "public/templates/instagram-reels-dark-minimal/hero.jpg"
      provides: "Hero card image asset referenced by template mediaAssets"
  key_links:
...
<objective>
Create the template type system and data infrastructure for theme templates, including placeholder asset files.

Purpose: Establish the strongly-typed data format that all templates follow, the registry for looking up templates by theme, one real placeholder template to validate the schema works end-to-end, and the asset files that the template references (so the apply route's fs.readFileSync won't throw and the picker shows real thumbnails).
Output: Template types, registry module, one instagram-reels template with realistic card data, and asset files on disk.
</objective>
...
<task type="auto">
  <name>Task 1: Create template type definitions and seed asset files</name>
  <files>src/lib/templates/types.ts, public/templates/instagram-reels-dark-minimal/thumbnail.jpg, public/templates/instagram-reels-dark-minimal/hero.jpg</files>
  <action>
...
**Part B: Create template asset directory and placeholder images.**

Create the directory `public/templates/instagram-reels-dark-minimal/` and seed it with two placeholder images:

1. **thumbnail.jpg** — Generate a simple solid-color placeholder image (dark background, ~400x600px). Use a Node.js script or sharp/canvas to create a minimal JPEG. If no image library is available, create a minimal valid JPEG file using a base64-encoded tiny dark image. The thumbnail must be a valid image file that Next.js Image component can render without errors.

2. **hero.jpg** — Same approach, a placeholder image (~800x400px or similar). Dark-toned to match the "Dark Minimal" template aesthetic.

These don't need to be beautiful — they just need to be valid image files so that:
- Plan 03's `fs.readFileSync(path.join(process.cwd(), 'public', 'templates', template.id, 'hero.jpg'))` won't throw
- Plan 04's template picker grid renders actual images (not broken img tags)

Alternative approach if image generation is difficult: download two small dark placeholder images via curl from a placeholder service like `https://placehold.co/400x600/1a1a1a/333333.jpg` and `https://placehold.co/800x400/1a1a1a/333333.jpg`. Or create them with ImageMagick (`convert -size 400x600 xc:#1a1a1a thumbnail.jpg`) if available.
  </action>
  <verify>Run `npx tsc --noEmit` — no type errors in the new file. Verify `ls public/templates/instagram-reels-dark-minimal/` shows both thumbnail.jpg and hero.jpg. Verify both files are non-empty valid images (`file public/templates/instagram-reels-dark-minimal/*.jpg` should report JPEG).</verify>
  <done>TemplateDefinition, TemplateCard, TemplateTheme, TemplateProfile exported from src/lib/templates/types.ts with correct imports from existing type system. public/templates/instagram-reels-dark-minimal/ directory exists with valid thumbnail.jpg and hero.jpg placeholder images.</done>
</task>
...
- `public/templates/instagram-reels-dark-minimal/thumbnail.jpg` exists and is a valid JPEG
- `public/templates/instagram-reels-dark-minimal/hero.jpg` exists and is a valid JPEG
</verification>
...
- Template asset files exist on disk (thumbnail.jpg, hero.jpg) as valid images
</success_criteria>
