...
2. **On click handler:**
   - Call `useThemeStore.getState().getSnapshot()` to get clean ThemeState
   - Call `usePageStore.getState().getSnapshot()` to get `{ cards, theme }`
...
Do NOT import from `src/lib/templates/types.ts` — the dev tool outputs raw JSON that the dev will paste into a template file manually. The TemplateDefinition type is the target format, but the snapshot doesn't need to import it.
  ...
  <done>DevTemplateSaver component exported. Returns null in production. Clicking the button captures all three store snapshots and copies formatted template JSON to clipboard with toast confirmation.</done>
</task>
...
- JSON output contains cards (without id/page_id/timestamps), theme snapshot, and profile defaults
</verification>

<success_criteria>
- Dev-only button gated behind NEXT_PUBLIC_DEV_TOOLS env flag
- Captures all three store states (page, theme, profile) via existing getSnapshot() methods
