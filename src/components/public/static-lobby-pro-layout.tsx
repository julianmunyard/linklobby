...
/**
 * Static Lobby Pro Layout for public pages
 * Client component for framer-motion animations
 */
export function StaticLobbyProLayout({
  username,
  title,
  cards,
  headingSize = 1.3,
  ...
  // Read border radius from CSS variable
  const borderRadius = 16 // Default from theme config
  ...
  const renderCardItem = (card: Card, index: number) => {
    switch (card.card_type) {
      ...
      case 'music': {
        const content = card.content as { thumbnailUrl?: string; trackTitle?: string; artistName?: string; platform?: string; embedUrl?: string }
        return (
          <AnimatedListItem key={card.id} index={index}>
            <a
              href={content?.embedUrl || card.url || '#'}
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full p-4 transition-all duration-200 hover:scale-[1.02]"
              style={glassStyle}
              data-card-id={card.id}
            >
              <div className="flex items-center gap-4">
      ...
      case 'video': {
        ...
        return (
          <AnimatedListItem key={card.id} index={index}>
            <a
              href={content?.embedUrl || content?.videoUrl || card.url || '#'}
              target="_blank"
              rel="noopener noreferrer"
              className="block w-full overflow-hidden transition-all duration-200 hover:scale-[1.02]"
              style={glassStyle}
              data-card-id={card.id}
            >
              {thumbnail ? (
      ...
      case 'gallery': {
        const content = card.content as { images?: Array<{ url: string; caption?: string; link?: string }> }
            ...
            >
              <div className="flex gap-2 overflow-x-auto p-3 snap-x snap-mandatory scrollbar-hide">
                {images.map((img, i) => (
                  ...
                  >
                    {img.link ? (
                      <a href={img.link} target="_blank" rel="noopener noreferrer" data-card-id={card.id}>
                        <Image
                          src={img.url}
        ...
        return (
          <AnimatedListItem key={card.id} index={index}>
            <a
              ...
              rel="noopener noreferrer"
              className={cn(
                'flex items-center gap-4 px-5 py-4 w-full transition-all duration-200 hover:scale-[1.02]',
              )}
              style={glassStyle}
              data-card-id={card.id}
            >
        ...
        return (
          <AnimatedListItem key={card.id} index={index}>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-4 px-5 py-4 w-full transition-all duration-200 hover:scale-[1.02]"
              style={glassStyle}
              data-card-id={card.id}
            >
              {thumbnail && (
