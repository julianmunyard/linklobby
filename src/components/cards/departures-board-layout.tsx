...
/**
 * Departures Board Layout - Renders cards as airport flight information display
 *
 * Features:
 * - Pure black background with dark navy card rows
 * - Thin horizontal ruled lines separating each row
 * - AuxMono monospace font (ALL CAPS)
 * - Tabular columns: TIME | DESTINATION | GATE | STATUS
 * - Section headers like "DEPARTURES" centered with rules above/below
 * - Silver/light grey text on dark background
 * - Status column shows "ON TIME" in accent color
 */
          ...
          <div className="departures-board-content">
            {visibleCards.map((card, index) => {
              ...
              if (card.card_type === 'audio' && isAudioContent(card.content)) {
                ...
                return (
                  <div key={card.id} className="px-4 py-3" onClick={() => onCardClick?.(card.id)}>
                    <AudioPlayer
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    focusedIndex === index && selectedCardId === card.id && "bg-white/5"
                  )}
